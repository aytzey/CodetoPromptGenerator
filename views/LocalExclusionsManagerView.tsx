  ...
  async function fetchLocalExclusions() {
    ...
    try {
      const resp = await fetch(
        `${BACKEND_URL}/api/localExclusions?projectPath=${encodeURIComponent(
          projectPath
        )}`
      );
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {localList.length > 0 && (
            <Badge
              variant="outline"
              className="bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 border-purple-200 dark:border-purple-800"
            >
              {localList.length}
            </Badge>
          )}
              ...
              {localList.map((item) => (
                ...
                >
                  <div className="flex items-center">
                    <FileX
                      size={14}
                      className="text-purple-400 dark:text-purple-500 mr-2 opacity-70"
                    />
