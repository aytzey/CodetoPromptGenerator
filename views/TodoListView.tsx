  ...
  async function clearCompleted() {
    ...
    try {
      ...
      for (const todo of completedTodos) {
        try {
          const resp = await fetch(
            `${BACKEND_URL}/api/todos/${todo.id}?projectPath=${encodeURIComponent(projectPath)}`,
            { method: 'DELETE' }
          );
          ...
          if (!data.success) {
            hasErrors = true;
            deletionErrors.push(data.error || `Failed to delete task #${todo.id}`);
          }
        } catch (err: any) {
          hasErrors = true;
          deletionErrors.push(`Error deleting task #${todo.id}: ${err.message}`);
        }
      }
      ...
      if (hasErrors) {
        ...
        loadTodos();
        setError(`Some tasks could not be deleted: ${deletionErrors.slice(0, 2).join(', ')}${deletionErrors.length > 2 ? '...' : ''}`);
      }
    } catch (err: any) {
              ...
              <SelectContent className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
                <SelectItem value="all" className="text-gray-800 dark:text-gray-200">
                  <div className="flex items-center">
                    <List size={14} className="mr-2 text-gray-500 dark:text-gray-400" />
                    All Tasks
                  </div>
                </SelectItem>
                <SelectItem value="active" className="text-gray-800 dark:text-gray-200">
                  <div className="flex items-center">
                    <ListTodo size={14} className="mr-2 text-blue-500 dark:text-blue-400" />
                    Active Tasks
                  </div>
                </SelectItem>
                <SelectItem value="completed" className="text-gray-800 dark:text-gray-200">
                  <div className="flex items-center">
                    <ListChecks size={14} className="mr-2 text-green-500 dark:text-green-400" />
                    Completed
                  </div>
                </SelectItem>
              </SelectContent>
