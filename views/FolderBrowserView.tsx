...
import { ChevronLeft, Folder, HardDrive, X, ChevronRight, Search, Home, Clock, Loader2, FolderOpen } from 'lucide-react'
...
const FolderBrowserView: React.FC<FolderBrowserProps> = ({
  ...
  const renderBreadcrumbs = () => {
    if (!path) return <span className="text-gray-500 dark:text-gray-400 italic">No folder selected</span>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-5xl h-[80vh] p-0 flex flex-col overflow-hidden bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800">
        ...
        {/* Error message */}
        {error && (
          <Alert variant="destructive" className="mx-4 mt-4 bg-rose-50 dark:bg-rose-950 border-rose-200 dark:border-rose-900 text-rose-800 dark:text-rose-300">
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <aside className="w-1/4 border-r border-gray-200 dark:border-gray-800 flex flex-col overflow-hidden">
            ...
            <div className="p-2 bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center">
              ...
              <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200">Drives</h4>
            </div>
                ...
                <div className="space-y-1">
                  {drives.map((drive) => (
                    <Button
                      ...
                      variant={selectedDrive === drive.path ? "secondary" : "ghost"}
                      className={`
                      ...
                      size="sm"
                    >
                      <HardDrive size={16} className={`mr-2 ${selectedDrive === drive.path ? 'text-indigo-500 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`} />
            ...
            <div className="p-2 bg-gray-100 dark:bg-gray-800 border-y border-gray-200 dark:border-gray-700 flex items-center">
              <Clock size={16} className="text-cyan-500 dark:text-cyan-400 mr-2" />
              <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200">Recent Folders</h4>
            </div>
            <ScrollArea className="flex-1 p-2 text-sm bg-white dark:bg-gray-900">
              {recentFolders.length === 0 ? (
                <div className="text-center p-4 text-gray-500 dark:text-gray-400 italic">No recent folders</div>
              ) : (
                <div className="space-y-1">
                  {recentFolders.map((folder) => (
                    <Button
                      ...
                      title={folder.path}
                    >
                      <Folder size={16} className="text-cyan-500 dark:text-cyan-400 mr-2" />
          ...
          <main className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-gray-900">
            ...
            <div className="p-2 bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between sticky top-0 z-10">
              <div className="flex items-center">
                <Folder size={16} className="text-amber-500 dark:text-amber-400 mr-2" />
                <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200">Folders</h4>
            ...
            <ScrollArea className="flex-1 bg-white dark:bg-gray-900">
              {isLoading ? (
                <div className="flex flex-col items-center justify-center h-full py-12">
                  <Loader2 size={32} className="animate-spin text-indigo-500 dark:text-indigo-400 mb-4" />
                ...
                <Table>
                  <TableHeader className="sticky top-0 bg-white dark:bg-gray-900 z-10">
                    <TableRow className="border-b border-gray-200 dark:border-gray-800">
                      <TableHead className="w-2/5 text-gray-700 dark:text-gray-300">Name</TableHead>
                      <TableHead className="w-3/5 hidden md:table-cell text-gray-700 dark:text-gray-300">Path</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
        ...
        <DialogFooter className="p-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
          <div className="mr-auto text-sm text-gray-500 dark:text-gray-400">
            {filteredFolders.length > 0 && (
              <Badge variant="outline" className="text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/30">
